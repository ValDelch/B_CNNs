...
class ExpE2SFCNN(torch.nn.Module):
    ...
    def __init__(self, n_channels, n_classes,
        ...
        self.LAYER += 1
        eq_layers += layer_builder(r1, int(scale*8), 9, 4)
        
        # 24 px
        # Convolutional Layer 2
        self.LAYER += 1
        eq_layers += layer_builder(eq_layers[-1].out_type, int(scale*16), 7, 3, 2)
        
        # 12 px
        # Convolutional Layer 3
        self.LAYER += 1
        eq_layers += layer_builder(eq_layers[-1].out_type, int(scale*24), 7, 3)
        
        # 12 px
        # Convolutional Layer 4
        self.LAYER += 1
        eq_layers += layer_builder(eq_layers[-1].out_type, int(scale*24), 7, 3, 2)
        
        # 6 px
        # Convolutional Layer 5
        self.LAYER += 1
        eq_layers += layer_builder(eq_layers[-1].out_type, int(scale*32), 7, 3)
        
        # 6 px
        # Convolutional Layer 6
        self.LAYER += 1
        eq_layers += layer_builder(eq_layers[-1].out_type, int(scale*40), 7, 0, None, True)
