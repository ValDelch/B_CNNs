...
class BesselConv2d(nn.Module):
    ...
    def __init__(self, k, C_in, C_out, strides=1, padding='VALID', reflex_inv=False, scale_inv=False, bias=True, 
        ...
        * When TensorCorePad is set to True, then padding is used to make mod(m_max+1,8)
        ...
        w_r_ini = np.random.normal(size=(self.m_max+1, self.j_max+1, self.C_in * self.C_out), loc=0., scale=np.sqrt(2./fan_in)*10)
        ...
        w_i_ini = np.random.normal(size=(self.m_max+1, self.j_max+1, self.C_in * self.C_out), loc=0., scale=np.sqrt(2./fan_in)*10)
        # Remove parameters when k_mj > k_max
        ...
        for i, scale in enumerate(self.scales):
            ...
            if not self.reflex_inv:
                ...
                if (self.padding == 'VALID' or self.padding == 0) and scale > 0:
                    ...
                    output = torch.square(
                        torch.nn.functional.conv2d(x[:,:,:,:], self.w[:,:,:,:], padding=self.padding, stride=self.strides)
                    )
                ...
                if (self.padding == 'VALID' or self.padding == 0) and scale > 0:
                    ...
                    output = torch.add(
                        torch.square(
                            torch.nn.functional.conv2d(x[:,:,:,:], self.w[:,:,:,:,0], padding=self.padding, stride=self.strides)
                        ),
                        torch.square(
                            torch.nn.functional.conv2d(x[:,:,:,:], self.w[:,:,:,:,1], padding=self.padding, stride=self.strides)
                        )
                    )
